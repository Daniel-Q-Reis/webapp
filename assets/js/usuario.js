$('#parar-de-seguir').on('click', pararDeSeguir);//Vao ser chamadas quando um dos botões da tela forem clicados
...
function pararDeSeguir() {
    const usuarioId = $(this).data('usuario-id');//pegar o valor do usuario ID
    ...
    $.ajax({
        ...
        window.location = `/usuarios/${usuarioId}`;//Aqui vou recarregar a pagina, ai vai trazer o usuario atualizado e já estaremos fora da lista de seguidores dele
    }).fail(function() {//assim o botão passara a ser o de seguir
...
function atualizarSenha(evento) {
    ...
    if ($('#nova-senha').val() != $('#confirmar-senha').val()) {//se as duas senhas novas não forem iguais, não vou nem mandar para a API
        Swal.fire("Ops...", "As senhas não coincidem!", "warning");
        return;
    }
    //A verificação se as duas senhas coincidem é feita na propria API, logo podemos executar esse ajax sem essa verificação
    $.ajax({
        url: "/atualizar-senha",
        method: "POST",
        data: {
